...
# # 학습셋:검증셋:테스트셋 = 7:2:1
ds = tf.data.Dataset.from_tensor_slices((padded_seqs, labels)) # data set 객체로 만듬
ds = ds.shuffle(len(features))    # 랜덤하게 섞음
train_size = int(len(padded_seqs) * 0.7)
val_size = int(len(padded_seqs) * 0.2)
test_size = int(len(padded_seqs) * 0.1)
train_ds = ds.take(train_size).batch(20)  # batch size
...
"""
Embeddig Layer - 희소벡터 -> 밀집벡터
                 단어 갯수, 결과 밀집벡터의 크기, 입력되는 시퀀스 크기, drop out(over fitting)
"""
# # # CNN 모델 정의
