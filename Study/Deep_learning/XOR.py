...
model = tf.keras.Sequential([   # 신경망 모델 정의
    ...
    tf.keras.layers.Dense(units= 1, activation= 'sigmoid')  # Dense - 모든 입력이 다 연결 # Sequential - 직선형구조 #출력 Layer
])
# compile - 학습 준비, 실행
...
history = model.fit(x, y, epochs=2000, batch_size=1) # 학습 옵션지정, epochs= 학습횟수, batch_size 반복되는 횟수
#
# model.predict(x) # 예측된 결과 값
